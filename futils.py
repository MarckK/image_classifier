...
def check_path(path):
    ...
    Returns filepath if valid, otherwise raises an error.
    """
...
def load_data(filepath='./flower', gpu=False):
    """
...
def model_setup(num_classes=102, pretrained_network='resnet50', learning_rate=0.001, hidden_units=None):
    ...
    checkpoint_data = {'in_features': in_features,
                       'hidden_layer1': hidden_layer1,
                       'hidden_layer2': hidden_layer2,
                       'output_layer': num_classes}
    
    return model, criterion, optimizer, checkpoint_data
...
def save_checkpoint(model, class_to_idx, checkpoint_data, filepath='checkpoint.pth'):
    """
    ...
    Returns:
        None
    """
    checkpoint = {'input_size': checkpoint_data['in_features'],
                'output_size': checkpoint_data['output_layer'],
                'hidden_layer1': checkpoint_data['hidden_layer1'],
                'hidden_layer2': checkpoint_data['hidden_layer2'],
                'dropout': 0.2,
...
def load_model(filepath='resnet50_model.pth'):
    ...
    end_index = filepath.find('_model.pth')
    arch = filepath[:end_index]
